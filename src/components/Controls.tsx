...
import { Palette, Type, Layout, MousePointer2, ChevronDown, ChevronUp } from 'lucide-react';
...
const Controls: React.FC<ControlsProps> = ({ theme, setTheme }) => {
  ...
  return (
    <div className={`flex flex-col bg-gray-900 text-sm overflow-hidden w-full transition-all duration-300 ${isCollapsed ? 'h-auto flex-shrink-0' : 'h-full'}`}>
      {/* Header */}
      <div className="p-4 border-b border-gray-800 flex-shrink-0 flex items-center justify-between">
        ...
        {/* Botón de colapsar solo visible en móviles */}
        <button 
